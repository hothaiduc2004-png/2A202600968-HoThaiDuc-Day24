...
# ─── Helper: single-answer correctness judge (dùng để so với human_label) ────

def judge_single_answer(question: str, model_answer: str) -> int:
    """Dùng LLM để đánh giá model_answer đúng/sai so với chính sách công ty.

    Trả về 1 (đúng/đầy đủ) hoặc 0 (sai/thiếu sót).
    Dùng cho Cohen's κ vs human_labels_10q.json (vốn là nhãn đúng/sai, không phải pairwise).
    """
    from openai import OpenAI
    client = OpenAI(api_key=OPENAI_API_KEY)
    prompt = f'''Bạn là expert đánh giá câu trả lời chính sách nhân sự.

Câu hỏi: {question}
Câu trả lời của model: {model_answer}

Câu trả lời này có chính xác và đầy đủ không?
Trả lời JSON (chỉ JSON): {{"correct": 1 hoặc 0, "reason": "..."}}
'''
    resp = client.chat.completions.create(
        model=JUDGE_MODEL,
        messages=[
            {"role": "system", "content": "Bạn là expert đánh giá chính sách HR. Chỉ trả lời JSON."},
            {"role": "user", "content": prompt},
        ],
        response_format={"type": "json_object"},
    )
    parsed = json.loads(resp.choices[0].message.content)
    return int(parsed.get("correct", 0))


def save_phase_b_report(kappa: float, bias: dict, demo: JudgeResult,
...
if __name__ == "__main__":
    ...
    print("Running LLM judge on the same 10 questions...")
    judge_labels = [
        judge_single_answer(item["question"], item["model_answer"])
    ...
    save_phase_b_report(kappa, bias, result)
